...
/**
 * a tool to handle locatization Chinese in jsp
 ...
 *
 */
public class LocalizationTool4j {
	final static String prefix="instancedetail_jsp";
	...
	public LocalizationTool4j(String filepath){
	...
	public static void main(String[] args) {
		...
		LocalizationTool4j tool = new LocalizationTool4j("d:/tmp/instancedetail.jsp");
		try {
			tool.start();
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 class  OKListener implements  ActionListener{
			...
			public void actionPerformed(ActionEvent arg0) {
				if(map.containsKey(key.getText()) && !value.getText().equals(map.get(key.getText()))){
					...
					System.out.print("<ww:text name=\""+key.getText()+"\"/>");
					outputProper.append("<ww:text name=\""+key.getText()+"\"/>");
					if(!(map.containsKey(key.getText()) && value.getText().equals(map.get(key.getText())))){
